//Sends info when user clicks validation link

import { hash } from '@server/services/hash_to_256';

export async function last_step_successful_stu_validation(IP_Address, user) {
  ...
  const serverEvent_0 = new ServerEvent()
    .setEventName('last_step_successful_stu_validation')
    .setEventTime(current_timestamp)
    .setUserData(userData_0)
    .setCustomData(customData_0)
    .setActionSource('website')
    .setEventSourceUrl('www.campuscanvas.net/auth/cuenta_verificada')
    .setEventId(uniqueID);
